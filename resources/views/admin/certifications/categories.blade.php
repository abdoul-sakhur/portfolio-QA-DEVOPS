...
@section('title', 'Catégories de certifications')

@section('content')
    <form method="POST" action="{{ route('admin.certifications.categories.store') }}" class="flex gap-3 mb-8">
        @csrf
        ...
        <table class="w-full text-sm">
            <thead class="border-b border-gray-800">
                <tr class="text-text-muted text-left">
                    ...
                    <th class="px-4 py-3">Certifications</th>
                ...
                @forelse($categories as $cat)
                    <tr class="border-b border-gray-800/50" x-data="{ editing: false }">
                        <td class="px-4 py-3">
                            ...
                            <form x-show="editing" x-cloak method="POST" action="{{ route('admin.certifications.categories.update', $cat) }}" class="flex gap-2">
                                @csrf @method('PUT')
                        ...
                        <td class="px-4 py-3 text-text-muted">{{ $cat->certifications_count }}</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-2">
                                ...
                                <x-modal-confirm>
                                    ...
                                    <form method="POST" action="{{ route('admin.certifications.categories.destroy', $cat) }}">
                                        @csrf @method('DELETE')

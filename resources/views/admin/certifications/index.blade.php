...
@section('content')
    <div class="flex items-center justify-between mb-6">
        ...
        <x-btn href="{{ route('admin.certifications.create') }}" size="sm">+ Nouvelle certification</x-btn>
    </div>
            ...
            <tbody>
                @forelse($certifications as $cert)
                    <tr class="border-b border-gray-800/50 hover:bg-bg-dark/50 transition">
                        <td class="px-4 py-3 text-text-main font-medium">{{ $cert->title }}</td>
                        <td class="px-4 py-3 text-text-muted">{{ $cert->issuer }}</td>
                        <td class="px-4 py-3">
                            <span class="text-xs bg-bg-dark text-accent px-2 py-1 rounded font-mono">{{ $cert->category->name ?? '—' }}</span>
                        </td>
                        <td class="px-4 py-3 text-text-muted text-xs">{{ $cert->issue_date?->format('d/m/Y') ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('admin.certifications.edit', $cert) }}" class="text-text-muted hover:text-accent transition text-xs">Modifier</a>
                                <x-modal-confirm>
                                    <x-slot:trigger>
                ...
                @empty
                    <tr><td colspan="5" class="px-4 py-8 text-center text-text-muted">Aucune certification.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

    <div class="mt-6">{{ $certifications->withQueryString()->links() }}</div>
@endsection
